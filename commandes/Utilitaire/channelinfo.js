...
module.exports.run = async (_bot, message, args) => {
	...
	if (channel.type === 'text' || channel.type === 'news' || channel.type === 'store') {
		...
		const embed = new MessageEmbed()
			.setColor(db.color)
			.setTitle(`Informations sur le salon textuel : ${channel.name}`)
			.setThumbnail(icon)
	...
	if (channel.type === 'voice') {
		...
		const embed = new MessageEmbed()
			.setThumbnail(icon)
			.setColor(db.color)
			.setTitle(`Informations sur le salon vocal : ${channel.name}`)
			.setFooter(message.member.user.username, message.member.user.displayAvatarURL())
	...
	return message.channel.send(`Erreur ${message.author.username}, je ne trouve pas le type du salon! Il m'est donc impossible d'afficher ses informations.`);
};
