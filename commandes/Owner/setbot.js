...
collector.on('collect', async r => { 
    if (r.emoji.name === '🇦') {
   message.channel.send(`🇦 Veuillez entrer le pseudonyme que vous voulez definir pour votre bot.`).then(mp => {
    ...
    // --
        ...
        message.channel.send(` 🇧 Veuillez entrer la photo de profil que vous voulez mettre pour le bot.`).then(mp => {
           ...
           message.delete()
            ...
            .catch(e => { return message.channel.send(` ${message.author}, Une erreur a été rencontrée. \n **Plus d'informations:** \`🔻\` \`\`\`${e}\`\`\``); });
