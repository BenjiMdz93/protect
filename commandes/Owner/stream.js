...
.then(p => message.channel.send(`<:yes:845285875466371073> ${message.author}, Vous avez défini le statut de votre bot en \`${str_content}\``))
.catch(e => { message.channel.send(`<:no:845285837259931697> ${message.author}, Une erreur est survenue. \n \`Plus d'informations:\` \`🔻\` \`\`\`${e}\`\`\``); });
    } else {
        message.channel.send(`<:no:845285837259931697> ${message.author}, Vous avez fourni aucune valeur.`);
    }
};
